...
void ApriltagDetector::detectTags(const cv::Mat& img_raw,
    std::vector<Eigen::Vector2d>& corners, std::vector<int>& ids, std::vector<double>& radii,
    std::vector<Eigen::Vector2d>& corners_rejected, std::vector<int>& ids_rejected, std::vector<double>& radii_rejected) {
  corners.clear();
  ids.clear();
  radii.clear();
