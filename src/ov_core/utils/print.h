...
public:
   ...
   * - PrintLevel::OVDEBUG : "OVDEBUG", "INFO", "WARNING" and "ERROR" will be printed. "ALL" will be silenced
   * - PrintLevel::INFO : "INFO", "WARNING" and "ERROR" will be printed. "ALL" and "OVDEBUG" will be silenced
   * - PrintLevel::WARNING : "WARNING" and "ERROR" will be printed. "ALL", "OVDEBUG" and "INFO" will be silenced
   * - PrintLevel::ERROR : Only "ERROR" will be printed. All the rest are silenced
   * - PrintLevel::SILENT : All PRINT_XXXX will be silenced.
   */
  enum PrintLevel { ALL = 0, OVDEBUG = 1, INFO = 2, WARNING = 3, ERROR = 4, SILENT = 5 };
...
void initSpdlog(std::string node_name, std::string &log_path, bool alsologtostderr = false);
...
// #define PRINT_DEBUG(x...) ov_core::Printer::debugPrint(ov_core::Printer::PrintLevel::OVDEBUG, __FILE__, TOSTRING(__LINE__), x);
